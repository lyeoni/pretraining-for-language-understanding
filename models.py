...
class LSTMLM(nn.Module):
    ...
    def forward(self, input):
        ...
        # |embeds| = (batch_size, seq_len-1, embedding_size)
        
        lstm_out, hidden = self.lstm(embeds)
        # If bidirectional=True, num_directions is 2, else it is 1.
        # |lstm_out| = (batch_size, seq_len-1, num_directions*hidden_size)
        # |hidden[0]|, |hidden[1]| = (num_layers*num_directions, batch_size, hidden_size)
        
        fc_out = self.fc(lstm_out)
        # |fc_out| = (batch_size, seq_len-1, hidden_size//2)
...
class BiLSTMLM(nn.Module):
    ...
    def forward(self, input):
        ...
        # |forward_out| = (batch_size, seq_len-3, hidden_size)
        # |backward_out| = (batch_size, seq_len-3, hidden_size)

        context = torch.cat((forward_out, backward_out), dim=-1)
        # |context| = (batch_size, seq_len-3, 2*hidden_size)
        
        fc_out = self.fc2(self.fc(context))
        # |fc_out| = (batch_size, seq_len-3, hidden_size)
        output = self.softmax(self.fc3(fc_out))
        # |output| = (batch_size, seq_len-3, output_size)
